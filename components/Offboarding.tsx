...
const Offboarding: React.FC<OffboardingProps> = ({ tasks: initialTasks }) => {
    ...
    const taskCategories = useMemo(() => {
        const categories: Record<string, OffboardingTask[]> = {};
    ...
    return (
        <div className="space-y-8">
            <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-md">
                <h3 className="text-2xl font-bold text-slate-900 dark:text-slate-50">{t('offboarding.title')}</h3>
                <p className="mt-2 text-slate-600 dark:text-slate-400">{t('offboarding.subtitle')}</p>
                <div className="mt-4">
                    <div className="flex justify-between mb-1">
                        <span className="text-base font-medium text-indigo-700 dark:text-indigo-400">{t('offboarding.progress')}</span>
            ...
            <div className="space-y-6">
             ...
             <div className="bg-slate-100 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-center">
                <p className="text-sm text-slate-600 dark:text-slate-400">
                    <Trans i18nKey="offboarding.contact_hr">
                        If you have any questions during this process, please contact the HR department at <a href="mailto:hr@example.com" className="text-indigo-600 dark:text-indigo-400 font-semibold hover:underline">hr@example.com</a>.
                    </Trans>
                </p>
            </div>
        </div>
